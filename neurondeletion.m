...
%% Cumulative Deletion of Neurons 
runiters = 1;
...
randint = randperm(200);
count = 1;

for r = randint
    max_store_change(r, :) = 10.*ones(1, size(max_store, 2));
    store_sum_change(r, :, :) = 5.*ones(1, size(store_sum, 2), size(store_sum, 3));
...
cum_delete(count) = (sum(char_store2 == char_store) / length(char_store) .*100);
count = count+1;
end 

delete_store(iter, :) = cum_delete;
end

%%
figure(1) 
mean_delete = mean(delete_store);
plot(1:23, delete_store(1:9:end))
xlabel("Years after Onset of Alzheimer's")
ylabel("Percentage of Original Message Retained")
title("Neuron Deletion")